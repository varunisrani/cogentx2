...
const QWEATHER_API_BASE_URL = process.env.QWEATHER_API_URL || "https://devapi.qweather.com/v7";
const QWEATHER_GEO_API_BASE_URL = process.env.QWEATHER_GEO_API_URL || "https://geoapi.qweather.com/v2";
...
const DEFAULT_FORECAST_DAYS = parseInt(process.env.WEATHER_FORECAST_DAYS || "3", 10);
// Create MCP server
...
// Add weather tool
server.tool("getWeather", "Get current weather information for a location. Returns weather data including temperature, humidity, and conditions.", {
    location: z
        .string()
        .optional()
        .describe("The location to get weather for (city name, zip code, coordinates, etc.). If not provided, default location will be used."),
    options: z
        .object({
        units: z
        ...
        language: z
            .string()
            .optional()
            .describe("Response language code (e.g., 'en' for English, 'es' for Spanish, 'zh' for Chinese)"),
    })
        .optional()
    ...
    try {
        ...
        weatherText += `Temperature: ${weatherData.temperature.toFixed(1)}${tempUnit}\n`;
        weatherText += `Feels Like: ${weatherData.feelsLike.toFixed(1)}${tempUnit}\n`;
        weatherText += `\nWind Information:\n`;
        ...
        if (error.response) {
            throw new Error(`Weather fetch failed: API responded with status ${error.response.status}`);
        }
...
// Add forecast tool
server.tool("getWeatherForecast", "Get weather forecast for a location. Returns weather forecast for the next few days.", {
    location: z
        .string()
        .optional()
        .describe("The location to get weather forecast for (city name, zip code, coordinates, etc.). If not provided, default location will be used."),
    options: z
        .object({
        units: z
            .enum(["metric", "imperial"])
        ...
        days: z
            .number()
            .optional()
            .describe("Number of days to forecast (default: 3, supports 3, 7days)"),
        language: z
            .string()
            .optional()
            .describe("Response language code (e.g., 'en' for English, 'zh' for Chinese)"),
    })
        .optional()
    ...
    try {
        ...
        const response = await axios.get(`${QWEATHER_API_BASE_URL}/weather/${forecastDays}d`, {
            params: {
                location: cityId,
                lang: mergedOptions.language,
            },
        ...
        const days = Math.min(mergedOptions.days || forecastDays, data.daily.length);
        const forecasts = [];
        ...
        for (let i = 0; i < days; i++) {
            if (i < data.daily.length) {
                ...
                const forecastDetails = generateWeatherForecast(dayData, mergedOptions.units === "imperial");
        ...
        if (error.response) {
            ...
            throw new Error(`Forecast fetch failed: API responded with status ${error.response.status}`);
        }
        else if (error.request) {
...
server.tool("getHourlyWeather", "Get hourly weather forecast for a location. Returns weather data hour by hour for the next 24 hours.", {
    location: z
        .string()
        .optional()
        .describe("The location to get hourly weather forecast for (city name, zip code, coordinates, etc.). If not provided, default location will be used."),
    options: z
        .object({
        units: z
            .enum(["metric", "imperial"])
        ...
        .optional()
        .describe("Hourly forecast configuration options, all fields are optional"),
}, async ({ location = "", options = {}, }) => {
    try {
        // Merge provided options with defaults from environment variables
        const mergedOptions = {
        ...
        for (let i = 0; i < hours; i++) {
            if (i < data.hourly.length) {
                ...
                const temp = isImperial
                    ...
                    : hour.temp;
                const dewPoint = isImperial && hour.dew
        ...
        // Handle case where the API request fails or returns an error
        if (error.response) {
            throw new Error(`Hourly forecast fetch failed: API responded with status ${error.response.status}`);
        }
        else if (error.request) {
            throw new Error(`Hourly forecast fetch failed: No response received from API`);
        }
        else {
...
async function resolveCityId(location, language = DEFAULT_LANGUAGE) {
    if (!location)
    ...
    try {
        ...
        const response = await axios.get(`${QWEATHER_GEO_API_BASE_URL}/city/lookup`, {
            params: {
                location,
                lang: language,
            },
...
server.tool("lookupCity", "Look up city information by name, ID, or coordinates. Returns city ID that can be used with other weather tools.", {
    location: z
        .string()
        .describe("The location to look up (city name, coordinates, etc.). Examples: 'London', 'New York', '39.9,116.3'"),
    options: z
        .object({
        language: z
            .string()
    ...
    try {
        ...
        data.location.forEach((city, index) => {
        ...
        // Handle case where the API request fails or returns an error
        if (error.response) {
            throw new Error(`City lookup failed: API responded with status ${error.response.status}`);
        }
        else if (error.request) {
...
function generateWeatherForecast(data, isImperial) {
    if (!data)
    ...
    forecast += `- Sunrise: ${data.sunrise || "N/A"}, Sunset: ${data.sunset || "N/A"}\n`;
    if (data.moonrise || data.moonset) {
        forecast += `- Moonrise: ${data.moonrise || "N/A"}, Moonset: ${data.moonset || "N/A"}\n`;
    }
    if (data.moonPhase) {
        forecast += `- Moon Phase: ${data.moonPhase} (Icon: ${data.moonPhaseIcon || "N/A"})\n`;
    }
    // Day weather section
    forecast += `\nDay Weather:\n`;
