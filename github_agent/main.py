...
def display_tool_usage(tool_usage):
    ...
    st.subheader("🔧 GITHUB TOOLS USED:")
...
async def main():
    ...
    with st.sidebar:
        ...
        1. Make sure the GitHub token has sufficient permissions
        2. Run 'bash setup_github_agent.sh' to install dependencies
        3. Check the log file for detailed error messages
        """)
    
    # Main query input
    user_query = st.text_area("💻 Enter your GitHub query:", height=100)
        ...
        try:
            with st.spinner("Processing your query..."):
                ...
                async with st.session_state.agent.run_mcp_servers():
                    result, elapsed_time, tool_usage = await run_github_query(st.session_state.agent, user_query)
            ...
            3. Run 'bash setup_github_agent.sh' to reinstall dependencies
            """)
