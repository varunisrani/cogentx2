...
from tools import run_github_query
...
def display_startup_message():
    ...
    print("1. Make sure the GitHub token has sufficient permissions")
    print("2. Run 'bash setup_github_agent.sh' to install dependencies")
...
async def main():
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"Could not detect Node.js: {str(e)}. Make sure Node.js is installed.")
        ...
        try:
            async with agent.run_mcp_servers():
                ...
                while True:
                    try:
                        ...
                        print(f"\nProcessing GitHub query: '{user_query}'")
                        print("This may take a moment...\n")
                        
                        # Run the query through the agent
                        try:
                            result, elapsed_time, tool_usage = await run_github_query(agent, user_query)
                            ...
                            print("\nSuggestions:")
                            print("1. Make sure your GitHub token has the necessary permissions")
            ...
            logger.error(f"Error details: {traceback.format_exc()}")
            print(f"\n❌ Error running GitHub MCP server: {str(server_error)}")
            print("\nTroubleshooting steps:")
            print("1. Make sure you have Node.js installed (version 16+)")
            print("2. Run 'bash setup_github_agent.sh' to install dependencies")
            print("3. Check that your GitHub token is valid and has proper permissions")
