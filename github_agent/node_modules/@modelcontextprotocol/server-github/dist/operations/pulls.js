...
import { GitHubPullRequestSchema, GitHubIssueAssigneeSchema, } from "../common/types.js";
// Schema definitions
...
export async function createPullRequest(params) {
    ...
    const response = await githubRequest(`https://api.github.com/repos/${owner}/${repo}/pulls`, {
        method: "POST",
        body: options,
    });
...
export async function getPullRequest(owner, repo, pullNumber) {
    const response = await githubRequest(`https://api.github.com/repos/${owner}/${repo}/pulls/${pullNumber}`);
    return GitHubPullRequestSchema.parse(response);
}
export async function listPullRequests(owner, repo, options) {
    ...
    if (options.head)
        url.searchParams.append('head', options.head);
    if (options.base)
        url.searchParams.append('base', options.base);
    if (options.sort)
...
export async function createPullRequestReview(owner, repo, pullNumber, options) {
    const response = await githubRequest(`https://api.github.com/repos/${owner}/${repo}/pulls/${pullNumber}/reviews`, {
        method: 'POST',
        body: options,
    });
...
export async function mergePullRequest(owner, repo, pullNumber, options) {
    return githubRequest(`https://api.github.com/repos/${owner}/${repo}/pulls/${pullNumber}/merge`, {
        method: 'PUT',
        body: options,
    });
}
export async function getPullRequestFiles(owner, repo, pullNumber) {
    const response = await githubRequest(`https://api.github.com/repos/${owner}/${repo}/pulls/${pullNumber}/files`);
    return z.array(PullRequestFileSchema).parse(response);
}
export async function updatePullRequestBranch(owner, repo, pullNumber, expectedHeadSha) {
    await githubRequest(`https://api.github.com/repos/${owner}/${repo}/pulls/${pullNumber}/update-branch`, {
        method: "PUT",
...
export async function getPullRequestComments(owner, repo, pullNumber) {
    const response = await githubRequest(`https://api.github.com/repos/${owner}/${repo}/pulls/${pullNumber}/comments`);
...
export async function getPullRequestStatus(owner, repo, pullNumber) {
    ...
    const response = await githubRequest(`https://api.github.com/repos/${owner}/${repo}/commits/${sha}/status`);
