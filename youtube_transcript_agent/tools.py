            ...
            if hasattr(server, 'session') and server.session:
                ...
                async def wrapped_call_tool(tool_name, arguments):
                    try:
                        ...
                        logging.debug(f"Raw tool result for {tool_name}: {result}")

                        # Fix for MCP validation error by ensuring 'content' field exists
                        if isinstance(result, dict):
                            # Case 1: toolResult contains content but top level doesn't
                            if 'toolResult' in result and isinstance(result['toolResult'], dict):
                                if 'content' in result['toolResult'] and 'content' not in result:
                                    result['content'] = result['toolResult']['content']
                                # Case 2: toolResult exists but no content anywhere
                                elif 'content' not in result:
                                    # Create a content field with a default value
                                    result['content'] = [{'type': 'text', 'text': str(result['toolResult'])}]
                            # Case 3: No content field at all
                            elif 'content' not in result:
                                # Create a content field with the entire result as text
                                result['content'] = [{'type': 'text', 'text': str(result)}]

                        # If result is not a dict, wrap it in a proper structure
                        ...
                        # If there's an error, return a properly formatted error response
                        logging.error(f"Error in wrapped_call_tool: {str(e)}")
                        return {
                            'content': [{'type': 'text', 'text': f"Error processing tool call: {str(e)}"}]
                        }
