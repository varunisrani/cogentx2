...
async def detect_mcp_needs(embedding_client: Optional[AsyncOpenAI], user_query: str) -> Dict[str, List[str]]:
    """
    ...
    # Perform semantic matching if no exact matches found and embedding_client is provided
    if not matched_services and embedding_client:
        try:
            logger.info("No keyword matches found, attempting semantic matching")
            # For now, we'll just use a simplified approach without embeddings
            # This avoids adding complexity while fixing the immediate issue

            # Check for general API or integration terms
            general_api_terms = ["api", "integration", "connect", "service", "external", "data"]
            ...
            if matched_general_terms:
                ...
                logger.info(f"Detected general API needs via terms: {', '.join(matched_general_terms)}")
        except Exception as e:
...
async def validate_template_for_common_issues(code_files: Dict[str, str]) -> List[str]:
    """
    Validate template files for common issues that cause runtime errors.

    Args:
        code_files: Dictionary of code files (models_code, agents_code, etc.)

    Returns:
        List of detected issues that need to be fixed
    """
    issues = []

    # Check models.py for common issues
    if "models_code" in code_files:
        models_code = code_files["models_code"]

        # Check for load_config function
        if "def load_config" not in models_code:
            issues.append("Missing load_config function in models.py")

        # Check for Config class (not AgentConfig)
        ...
        if ".dict(" in models_code:
            issues.append("Using deprecated dict() method instead of model_dump() in models.py")

    # Check agents.py for common issues
    ...
    if "main_code" in code_files:
        main_code = code_files["main_code"]

        # Check for run_server vs run_mcp_servers
        if "agent.run_server()" in main_code:
            issues.append("Using incorrect run_server() method instead of run_mcp_servers() in main.py")

        # Check for import paths
        if "from models import load_config, initialize_agent" in main_code:
            issues.append("Incorrect import path for initialize_agent in main.py (should be from agents)")
...
5. CRITICAL ISSUES TO PREVENT:
- Always include load_config function in models.py
- Use Config class name (not AgentConfig) in models.py
- Include GitHubDeps class in models.py
- Use model_dump() method instead of deprecated dict() method
- Include spotify_system_prompt and github_system_prompt in agents.py
- Implement setup_spotify_agent and setup_github_agent functions
- Add recommend_songs_from_spotify and manage_github_repository functions
- Use run_mcp_servers() method (not run_server())
- Import initialize_agent from agents (not from models)
...
"""
        ...
        # 3. Validate the merged code using our comprehensive validation function
        validation_errors = await validate_template_for_common_issues(merged_files)
        ...
        if validation_errors:
            ...
            # Group errors by file type for more efficient fixing
            errors_by_file = {
                "models.py": [],
                "agents.py": [],
                "tools.py": [],
            ...
            for error in validation_errors:
                for file_type in errors_by_file.keys():
                    if file_type in error:
                        errors_by_file[file_type].append(error)

            # Fix each file with issues
            for file_type, errors in errors_by_file.items():
                if not errors:
                    continue

                code_key = file_type.replace(".py", "_code")
                if code_key in merged_files and errors:
                    fix_prompt = f"""Fix the following critical issues in the {file_type} code:
{chr(10).join('- ' + err for err in errors)}

These issues cause runtime errors in the agent. Please fix ALL of them.

Current code:
{merged_files[code_key]}
...
"""
                    ...
                    fix_response = await llm_client.chat.completions.create(
                        model="gpt-4o",
                        messages=[
                            {"role": "system", "content": "You are a Python expert fixing code integration issues in agent templates. You must fix ALL the listed issues completely."},
                    ...
                    merged_files[code_key] = fixed_code.strip()
                    logger.info(f"Fixed {len(errors)} issues in {file_type}")

            # Validate again to ensure all issues are fixed
            remaining_errors = await validate_template_for_common_issues(merged_files)
            if remaining_errors:
                logger.warning(f"Some issues could not be fixed automatically: {remaining_errors}")
                # Try one more time with more explicit instructions
                for file_type, errors in errors_by_file.items():
                    code_key = file_type.replace(".py", "_code")
                    remaining_file_errors = [err for err in remaining_errors if file_type in err]

                    if code_key in merged_files and remaining_file_errors:
                        fix_prompt = f"""URGENT: These critical issues in {file_type} MUST be fixed:
{chr(10).join('- ' + err for err in remaining_file_errors)}

These issues WILL cause runtime errors. You MUST fix ALL of them exactly as specified.
...
"""
                        ...
                        fix_response = await llm_client.chat.completions.create(
                            model="gpt-4o",
                            messages=[
                                {"role": "system", "content": "You are a Python expert fixing critical code issues. You must implement EXACTLY what is requested."},
                                {"role": "user", "content": fix_prompt}
                            ]
                        )
                        ...
                        merged_files[code_key] = fixed_code.strip()
                        logger.info(f"Second attempt to fix {len(remaining_file_errors)} issues in {file_type}")

        # 4. Create unified system prompt
        system_prompts = [await extract_system_prompt(t.get("agents_code", "")) for t in templates]
        merged_files["unified_system_prompt"] = await create_unified_prompt_with_ai(system_prompts, llm_client)
...
async def rule_based_merge_mcp_templates(templates: List[Dict[str, Any]], user_query: str) -> Dict[str, Any]:
    """
    Merge multiple MCP templates using rule-based approach.
    This is a fallback method when AI-based merging is not available or fails.
    ...
    # Initialize merged files dictionary
    merged_files = {
        "folder_name": folder_name,
        "purpose": f"Rule-based merged template for: {user_query}",
        "metadata": {
            "merge_method": "rule-based",
            "created_at": datetime.now().isoformat(),
            "original_query": user_query,
            "source_templates": [t.get("folder_name") for t in templates],
            "template_count": len(templates)
    ...
    for file_type in ["agents_code", "models_code", "main_code", "tools_code", "requirements_code", "env_example_code", "readme_code", "package_json_code"]:
        ...
        for template in templates:
            if template.get(file_type):
                code_blocks.append(template.get(file_type))

        if code_blocks:
            # Simple concatenation with separators
            merged_files[file_type] = "\n\n# ===== MERGED CODE SECTION =====\n\n".join(code_blocks)

    # Merge MCP JSON
    merged_mcp = await merge_mcp_json(templates, user_query)
    merged_files["mcp_json"] = json.dumps(merged_mcp, indent=2)
...
IMPORTANT: Ensure your code avoids these common issues that cause runtime errors:
...
"""
    ...
    # Final validation to ensure no common issues remain
    if llm_client:
        try:
            logger.info("Performing final validation of adapted template...")
            validation_errors = await validate_template_for_common_issues(adapted_template)

            if validation_errors:
                logger.warning(f"Found {len(validation_errors)} issues in adapted template: {validation_errors}")
                logger.info("Fixing remaining issues...")
                ...
                for file_type, errors in errors_by_file.items():
                    ...
                    code_key = file_type.replace(".py", "_code")
                    if code_key in adapted_template and errors:
                        fix_prompt = f"""CRITICAL: Fix these issues in the {file_type} code:
{chr(10).join('- ' + err for err in errors)}

These issues WILL cause runtime errors. You MUST fix ALL of them exactly as specified.

Current code:
{adapted_template[code_key]}
...
"""
                        ...
                        adapted_template[code_key] = fixed_code.strip()
                        logger.info(f"Fixed {len(errors)} issues in {file_type}")

                # Add a note to the README about fixes
                if "readme_code" in adapted_template:
                    adapted_template["readme_code"] += "\n## Automatic Fixes\n\nThis template was automatically validated and fixed to prevent common runtime errors.\n"

                logger.info("Template validation and fixing complete")
        except Exception as e:
