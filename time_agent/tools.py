...
async def display_mcp_tools(server: MCPServerStdio):
    """Display available Time MCP tools and their details"""
    try:
        logging.info("\n" + "="*50)
        logging.info("TIME MCP TOOLS AND FUNCTIONS")
        logging.info("="*50)
        
        # Try getting tools through server's session
        tools = []
        try:
        ...
        if tools:
            # Display tools and their details
            for tool in tools:
                logging.info(f"\n📌 {tool.get('name')}")
                logging.info("   " + "-"*40)
            ...
            logging.info("\nExample Queries:")
            logging.info("-"*50)
            logging.info("1. 'What time is it in Tokyo?'")
            logging.info("2. 'Convert 3:30 PM EST to PST'")
            logging.info("3. 'How many hours between 2pm and 8pm?'")
            logging.info("4. 'Format 2025-04-08 14:30:00 as RFC3339'")
            
        else:
            logging.warning("\nNo Time MCP tools were discovered. This could mean either:")
            logging.warning("1. The Time MCP server doesn't expose any tools")
...
async def run_time_query(agent, user_query: str) -> tuple:
...
def create_mcp_server():
    ...
    return MCPServerStdio(
        'python',
        ['-m', 'mcp_server_time', '--local-timezone=America/New_York']
    )
