...
SPOTIFY_KEYWORDS = ['spotify', 'music', 'playlist', 'song', 'track', 'artist', 'album']
FIRECRAWL_KEYWORDS = ['crawl', 'spider', 'scrape', 'extract', 'website', 'webpage']
...
async def display_mcp_tools(server: MCPServerStdio) -> List[Dict[str, Any]]:
...
def create_spotify_mcp_server(SPOTIFY_API_KEY: str) -> MCPServerStdio:
    """Create an MCP server for SPOTIFY.
    
    Args:
        SPOTIFY_API_KEY: API key for SPOTIFY service
...
def create_firecrawl_mcp_server(FIRECRAWL_API_KEY: str) -> MCPServerStdio:
    """Create an MCP server for FIRECRAWL.
    ...
    try:
        ...
        logging.error(f"Error creating FIRECRAWL MCP server: {str(e)}")
        logging.error(f"Error details: {traceback.format_exc()}")
        raise


async def run_spotify_query(query: str, agent: Agent) -> str:
    """Execute a query using the SPOTIFY service.
    ...
    """
    try:
        # Find the SPOTIFY server
        server = next((s for s in agent.mcp_servers if "mcp-server-spotify" in str(s.args)), None)
        if not server:
            raise ValueError("No SPOTIFY server found")
        ...
        logging.error(f"Error running SPOTIFY query: {e}")
        return f"Error: {str(e)}"


async def run_firecrawl_query(query: str, agent: Agent) -> str:
    """Execute a query using the FIRECRAWL service.
    ...
    """
    try:
        # Find the FIRECRAWL server
        server = next((s for s in agent.mcp_servers if "mcp-server-firecrawl" in str(s.args)), None)
        if not server:
            raise ValueError("No FIRECRAWL server found")
        ...
        logging.error(f"Error running FIRECRAWL query: {e}")
        return f"Error: {str(e)}"

async def run_query(query: str, agent: Agent) -> str:
    ...
    try:
        ...
        # Check if query is relevant for SPOTIFY
        if any(keyword in query_lower for keyword in SPOTIFY_KEYWORDS):
            logging.info("Using SPOTIFY for query")
            return await run_spotify_query(query, agent)
            
        # Check if query is relevant for FIRECRAWL
        if any(keyword in query_lower for keyword in FIRECRAWL_KEYWORDS):
            logging.info("Using FIRECRAWL for query")
            return await run_firecrawl_query(query, agent)
            
        # If no specific service matches, try the most appropriate one
        ...
        try:
            ...
            try:
                ...
                logging.debug(f"Failed to run query with SPOTIFY: {e}")
                
            try:
                return await run_firecrawl_query(query, agent)
            except Exception as e:
                logging.debug(f"Failed to run query with FIRECRAWL: {e}")
