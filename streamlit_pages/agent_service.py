...
def agent_service_tab():
    ...
    # Also add logs to the system logs for centralized viewing
    if st.session_state.service_output and "system_logs" in st.session_state:
        for log in st.session_state.service_output:
            if log not in st.session_state.system_logs:
                st.session_state.system_logs.append(log)