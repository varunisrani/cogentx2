...
def get_thread_id():
    ...
    # Only generate a new one if it doesn't exist in the session state
...
async def run_agent_with_streaming(user_input: str):
    ...
    logging.info(f"Using thread ID: {thread_id} for conversation")
...
async def chat_tab():
    ...
    # Ensure thread_id is initialized
    _ = get_thread_id()  # Just ensure it's initialized, we don't need to use the value here

    # Add a clear conversation button
    if st.button("Clear Conversation"):
        # Clear messages and thread_id to start a fresh conversation
        st.session_state.messages = []
