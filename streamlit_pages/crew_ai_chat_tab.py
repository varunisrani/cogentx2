...
def _get_mcp_tool_flow():
    ...
    if _mcp_tool_flow is None:
        try:
            ...
            try:
                from streamlit_pages.mcp_tools.mcp_tool_graph import combined_adaptive_flow
                _mcp_tool_flow = combined_adaptive_flow
                logger.info("Successfully imported MCP tool flow using absolute import")
            except ImportError:
                # Fallback to relative import
                from .mcp_tools.mcp_tool_graph import combined_adaptive_flow
                _mcp_tool_flow = combined_adaptive_flow
                logger.info("Successfully imported MCP tool flow using relative import")
        except ImportError as e:
            logger.warning(f"Failed to import MCP tool flow: {str(e)}")
            return None
...
# Expose the display_chat_interface function as crew_ai_chat_tab for external use
crew_ai_chat_tab = display_chat_interface

logger.info("crew_ai_chat_tab.py is being executed")